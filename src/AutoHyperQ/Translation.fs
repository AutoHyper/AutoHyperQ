            ...
            | RelationalEq(e1, n1, e2, n2) -> 
                ...
                let t = 
                    match NuSMV.VariableType.intersectTypes t1 t2 with 
                        | Some x -> x 
                        | None -> 
                            raise <| AutoHyperQException $"Error during unfolding: Could not intersect types %A{t1} and %A{t2} of expressions %A{e1} and %A{e2}."
            ...
            |> List.iter (fun (v : Expression) ->
                ...
                |> Set.iter (fun x ->
                        if (Set.contains x plist.[0].Vars |> not) &&  (plist.[0].Define.Keys.Contains x |> not) then
                            raise <| AutoHyperQException $"Variable %A{x} is used in the formula but not defined in the system"
                    )
            )
                ...
                |> List.iter (fun (v : Expression) ->
                    ...
                    |> Set.iter (fun x ->
                            if Set.contains x plist.[nameMapping[n]].Vars |> not && (plist.[nameMapping[n]].Define.Keys.Contains x |> not) then
                                raise <| AutoHyperQException $"Variable %A{x}, %s{n} is used in the formula but defined in the system for trace variable %s{n}"
                        )
                )
        ...
        if progList.Length = 1 then 
            ...
            |> List.iter (fun (v, i) ->
                if prog.DomainMap.ContainsKey v && prog.DomainMap.[v] > i |> not then
                    raise <| AutoHyperQException $"AP (%A{v}, %i{i}) is used in the HyperQPTL property but variable %A{v} does not exists or has not the required bit width. Aborting."
                )
            ...
            |> List.map (fun n ->   
                ...
                |> List.iter (fun (v, j) ->
                    if progList.[nameMapping[n]].DomainMap.ContainsKey v && progList.[nameMapping[n]].DomainMap.[v] > j |> not then
                        raise <| AutoHyperQException $"AP (%A{v}, %i{j}) is used in the HyperQPTL property but variable %A{v} does not exists or has not the required bit width. Aborting."
                    )
