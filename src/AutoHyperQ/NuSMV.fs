        ...
        match e with 
                    ...
                    | SetValue x -> 
                        ...
                        |> List.reduce (fun x y -> 
                            match VariableType.joinTypes x y with 
                                | Some x -> x 
                                | None -> 
                                    raise <| AutoHyperQException "Error during NuSMV-to-Explicit compilation. Failed to join types."
                            )
                        |> SetType
            | Eq _ -> 
                BoolType
            ...
            | And(e1, e2) -> 
                match inferType env e1, inferType env e2  with 
                    ...
                    | _ ->
                        raise <| AutoHyperQException "Error during NuSMV-to-Explicit compilation."
            | Or(e1, e2) -> 
                match inferType env e1, inferType env e2  with 
                    | BoolType, BoolType -> BoolType
                    | _ ->
            ...
            | SetExp e -> 
                ...
                |> List.reduce (fun x y -> 
                        match VariableType.joinTypesAndExtend x y with 
                            | Some x -> x 
                            | None -> 
                                raise <| AutoHyperQException $"Error during NuSMV-to-Explicit compilation. Failed to join types %A{x} and %A{y}."
                        )
                |> SetType 
            | Case (cases) -> 
    ...
    let rec eval (env : String -> VariableValue) (e : Expression) = 
        match e with 
            ...
            | Leq(e1, e2) -> 
                ...
                match v1, v2 with 
                    | IntValue i1, IntValue i2 -> 
                        BoolValue (i1 <= i2)
                    | _ -> 
                        raise <| AutoHyperQException "Error during NuSMV-to-Explicit compilation. Invalid Comparison."
            | Geq(e1, e2) -> 
                let v1 = eval env e1 
                let v2 = eval env e2 
                match v1, v2 with 
            ...
            | And(e1, e2) -> 
                ...
                match v1, v2 with 
                    | BoolValue b1, BoolValue b2 -> 
                        BoolValue (b1 && b2)
                    | _ -> 
                        raise <| AutoHyperQException $"Error during NuSMV-to-Explicit compilation. & is not supported for values {{%A{v1}}} and {{%A{v2}}}"
            | Or(e1, e2) -> 
                let v1 = eval env e1 
                let v2 = eval env e2 
                match v1, v2 with 
            ...
            | Neg e ->
                ...
                match v with 
                    | BoolValue b -> 
                        BoolValue (not b)
                    | _ -> 
                        raise <| AutoHyperQException $"Error during NuSMV-to-Explicit compilation. ! is not supported for values {{%A{v}}}"
            | Add(e1, e2) -> 
                let v1 = eval env e1 
                let v2 = eval env e2 
                match v1, v2 with 
            ...
            | Case cases -> 
                ...
                match firstHit with 
                    | Option.None ->
                        raise <| AutoHyperQException "Error during NuSMV-to-Explicit compilation. No case was matched."
                    | Some (_, e) -> 
                        eval env e
    ...
    let rec evalWithDefinitions state (e : Expression) = 
        ...
        |> Expression.eval (fun x -> 
            if Set.contains x p.Vars then 
                ...
                raise <| AutoHyperQException $"Did not find a definition for |%s{x}|"
            )
                    ...
                    match v with 
                        | SetValue v -> 
                            if List.forall (fun x -> VariableType.isValueOfType x p.VarTypes.[name]) v then 
                                ...
                                raise <| AutoHyperQException $"The value of the next expression {{%A{v}}} does not match its intended type {{%A{p.VarTypes.[name]}}}"
                        | _ -> 
                            raise <| AutoHyperQException $"The value of the next expression {{%A{v}}} does not match its intended type {{%A{p.VarTypes.[name]}}}"
    ...
    let private programParser = 
        pipe4
            ...
            (fun _ var (init, next) define ->   
                ...
                |> List.iter (fun (n, c) -> 
                    if c > 1 then 
                        raise <| AutoHyperQException $"Error while parsing NuSMV Model: The initial condition for variable %s{n} is defined multiple times."
                    )
                ...
                |> List.iter (fun (n, c) -> 
                    if c > 1 then 
                        raise <| AutoHyperQException $"Error while parsing NuSMV Model: The next condition for variable %s{n} is defined multiple times."
                    )
                ...
                
                {
                    ...
                    SmvProgram.Init = 
                        ...
                        |> List.map 
                            (fun (n, e) -> 
                                n, e |> Expression.eval (fun x -> raise <| AutoHyperQException $"Error while parsing NuSMV Model: Only closed expression in init conditions. %s{x} is unbound in %s{n}, %A{e}")
                            )
                        |> Map.ofList
